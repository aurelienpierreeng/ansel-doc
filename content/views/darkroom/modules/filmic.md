...
: With the _v8 (AgX-like)_ color science, this slider is relabelled _color preservation_ and drives the balance between chromatic character and color fidelity. From zero (the default) upward, the original hues are always fully preserved, and positive values progressively restore the original chroma, measured in a perceptual color space -- keeping bright legitimate colors such as sunsets or blue skies against orange clouds saturated. Negative values fade in the per-channel hue drifts and increase the bleaching (the stronger "film" character). The hue is deliberately restored before the chroma: restoring saturation onto a drifted hue would produce visible color casts in gradients. Chroma-wise, zero restores half of the original, mirroring the _v7_ "equal mix" convention. See the [background](#background) section.
...
- **Color fidelity is recoverable.** AgX gives you a hue mix performed in HSV (a non-perceptual space) and no way to keep legitimate bright colors saturated. In _v8_, the _color preservation_ slider spans a continuum from full per-channel character to ratio-preserving color at per-channel lightness, with chroma and hue restored in a perceptual color space -- hue first, so that restored saturation never lands on a drifted hue. At the default position the hues are already fully preserved and half of the original chroma is restored.
