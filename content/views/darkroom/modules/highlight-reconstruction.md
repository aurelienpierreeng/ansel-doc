...
If you find yourself having to increase the _diameter of reconstruction_ past 512px to get a full recovery from magenta, cap the diameter to 512px, do the most you can with this setting, and then **add a second instance of this module** to finish the work, masked to the area that still needs it. This gives more bearable run-times with a very similar result.

Older versions of this page advised enabling filmic's highlight reconstruction at that point instead. That is no longer recommended: [filmic's reconstruction is deprecated](./filmic.md#reconstruct) and disabled for new edits, since this module now covers X-Trans and already-demosaiced input too, and its default _harmonic transposition_ method is more accurate than filmic's first-order color propagation. The current recommendation is to stay in this module: keep _harmonic transposition_ as the base, and stack a _guided laplacians_ instance on top of it if harmonic transposition alone is not enough.
